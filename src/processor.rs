...
use crate::instruction::{CleanUp, CloseTradeMarket, DecommissionTrader, InitializeTradeMarket, MarketStatus, RegisterTrader, Settle, Trade, TradeBotInstruction, UpdateTrader};
...
impl Processor {
    ...
    pub fn process_cleanup_decommissioned_trader(program_id: &Pubkey, accounts: &[AccountInfo], ix: &CleanUp) -> TradeBotResult<()> {
        let accounts_iter = &mut accounts.iter();
        ...
        let open_orders = serum_market.load_orders_mut(serum_open_orders_account, Some(trader_signer_account), serum_program.key, None, None).unwrap();

        let base_trader_wallet_account = spl_token::state::Account::unpack(&mut base_trader_wallet.try_borrow_mut_data().unwrap()).unwrap();
        let quote_trader_wallet_account = spl_token::state::Account::unpack(&mut quote_trader_wallet.try_borrow_mut_data().unwrap()).unwrap();
        if my_orders.len() == 0 {
            if open_orders.native_pc_free == 0 && open_orders.native_pc_total == 0
            && open_orders.native_coin_free == 0 && open_orders.native_coin_total == 0 {
                if quote_trader_wallet_account.amount == 0 && quote_trader_wallet_account.amount == 0{
    ...
    pub fn process_trade(program_id: &Pubkey, accounts: &[AccountInfo], ix: &Trade) -> TradeBotResult<()> {
        ...
        let profit_as_base = trader.min_trade_profit / sell_price;
        let base_size = trader.base_balance  / (trader.simultaneous_open_positions - trader.open_order_pairs * 2);

        let quote_size = base_size  * sell_price;
        msg!("{} {} {}", quote_size, base_size, profit_as_base);
        msg!("{} {} ", serum_market.pc_lot_size, serum_market.coin_lot_size);

        let order_buy_price = (quote_size / (base_size + (profit_as_base / 2) + (0.0022_f64 * base_size as f64) as u64));
        let order_sell_price = (( (0.0022_f64 * quote_size as f64) as u64) + quote_size + trader.min_trade_profit / 2) / base_size;
        msg!("{} {} {}", order_buy_price, order_sell_price, trader.min_trade_profit);
        ...
        msg!("{} {}", base_size_lots, quote_size_lots);
        // return Ok(());
    ...
    pub fn process_settle(program_id: &Pubkey, accounts: &[AccountInfo], ix: &Settle) -> TradeBotResult<()> {
        ...
        let mut accounts1: Vec<AccountMeta> = vec![
            ...
            AccountMeta::new(base_trader_wallet.key.clone(), false),
            AccountMeta::new(quote_trader_wallet.key.clone(), false),
        ...
        let serum_market = serum_dex::state::Market::load(market, serum_program.key, true).unwrap();
        let bids = Self::parse_order_book_for_owner(Side::Bid, serum_market.load_bids_mut(bids_account).unwrap().deref().clone(), &serum_open_order_as);
        let asks = Self::parse_order_book_for_owner(Side::Ask, serum_market.load_asks_mut(asks_account).unwrap().deref().clone(), &serum_open_order_as);
        let open_orders = serum_market.load_orders_mut(serum_open_orders, Some(trader_signer), serum_program.key, None, None).unwrap();
        let base_trader_wallet_account = spl_token::state::Account::unpack(&mut base_trader_wallet.try_borrow_mut_data().unwrap()).unwrap();
        let quote_trader_wallet_account = spl_token::state::Account::unpack(&mut quote_trader_wallet.try_borrow_mut_data().unwrap()).unwrap();
        let mut all_bids = Self::parse_order_book(Side::Bid, serum_market.load_bids_mut(bids_account).unwrap().deref().clone());


        let buy_price = all_bids.get(0).unwrap().price;
        trader.base_balance += open_orders.native_coin_free;
        trader.quote_balance += open_orders.native_pc_free;
        trader.value = ((base_trader_wallet_account.amount / serum_market.coin_lot_size  + open_orders.native_coin_total)* buy_price) + (open_orders.native_pc_total + quote_trader_wallet_account.amount);
